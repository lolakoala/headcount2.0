 class DistrictRepository {
   ...
   cleanData(dirtyData) {
    //  cleans the data coming in - becomes an array of objects with
    // location and kidsInSchool as properties so each objects collects
    ...
    //  let locationObjectArray = [];
     let locationArray = dirtyData.reduce((acc, elem) => {
       if (!acc.includes(elem.Location)) {
         acc.push(elem.Location);
       }
       return acc;
     }, []);
     let locationObjectArray = locationArray.reduce((acc, district) => {
       acc.push({
         'location': district.toUpperCase(), 'kidsInSchool': { } });
       return acc;
     }, []);
